...
@section('content')
    <div class="layout-container">
        ...
        <h2 class="sectionTitle">{{ trans("articles.new_$section") }}</h2>

        <form class="articleForm js-articleForm" method="post" action="{{ route('articles.store', ['section' => $section]) }}" @submit.prevent="saveForm">
            {{ csrf_field() }}
                ...
                @foreach (config('app.available_locales') as $language)
                    <div role="tabpanel" class="tab-pane {{ $language === 'fr' ? 'active' : '' }}" id="{{ $language }}">
                        ...
                        <div class="form-group">
                            ...
                            <image-upload :src="article.{{ $language }}.image" upload-url="/admin/articles/{{ $section }}/upload" @update="updateThumbnail" @changeState="changeState" language="{{ $language }}" label="Téléverser une image">
                        </div>
            ...
            <input class="btn btn-primary" type="submit" value="{{ trans('articles.save') }}">
        </form>
    </div>
@endsection
...
@push('scripts')
    ...
    <template id="blockImageTemplate">
        <div class="block blocks-template">
            ...
            <div class="row">
                <div class="col-md-6">
                    <dropzone :src="image0" upload-url="/admin/articles/{{ $section }}/upload" @update="updateImage0Upload" @changeState="changeState" :language="language" label="Téléverser une image">
                </div>
                <div class="col-md-6">
                    <dropzone :src="image1" upload-url="/admin/articles/{{ $section }}/upload" @update="updateImage1Upload" @changeState="changeState" :language="language" label="Téléverser une image">
                </div>
            </div>
        </div>
    </template>
    ...
    <template id="blockTextImageTemplate">
        <div class="block blocks-template">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <dropzone :src="image" upload-url="/admin/articles/{{ $section }}/upload" @update="updateImageUpload" @changeState="changeState" :language="language" label="Téléverser une image">
                </div>
            </div>
        </div>
    </template>
        ...
        new Vue({
            ...
            methods: {
                ...
                saveForm: function () {
                    ...
                    if (this.state === 'ready') {
                        this.state = 'saving';
                        this.$http.post(this.$el.action, article).then(function (response) {
                            self.state = 'ready';
